...
# Dumped Test::More and just use eval to test modules. Thanks Perlmonks!
    ...
    # replaced Digest::SHA1 and SHA2 as of 07/2013, now Digest::SHA
    #  and in core modules

    my %module = (
        'dbi|DBI'         => '0:Database Connection',
        'dig|Digest::SHA' => '0:Digest Module for SHA1 and SHA2',
 ...
 #       'gds|GD::Graph::sparklines' => '2:Graphics for Graphs',
    ...
    # can be SHA1 or SHA2
    # replaced to Digest::SHA should be in core Perl now
    if ( $counter{'dig'} == 0 ) {
        $usable = 0;
        $diagnosis .=
"<li class=\"failedcheck\">cclite needs Digest::SHA which replaces Digest::SHA1 and Digest::SHA2</li>";
    }
